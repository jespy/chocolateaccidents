(function(){
  ...
  const keys={},interactables=[],blockers=[];let yaw=Math.PI,pitch=0,activeExamine=null,mobileMoveX=0,mobileMoveY=0,mobileSprint=false;const examineStack=[];
  ...
  function closeAll(captureMouse=true){screens.forEach(s=>$(s).classList.remove('active'));modal=false;suppressPause=true;if(captureMouse&&!touchMode)renderer.domElement.requestPointerLock()}
  ...
  function applyLook(dx,dy,sensitivity=.0022){yaw-=dx*sensitivity;pitch-=dy*sensitivity;pitch=Math.max(-1.45,Math.min(1.45,pitch));camera.rotation.set(pitch,yaw,0)}
  function performInteract(){if($('examine').classList.contains('active')){backExamine(false);return}if(!modal&&current)current.userData.interaction()}
  function toggleEvidence(){if($('journal').classList.contains('active'))closeAll(false);else if(!modal){renderJournal();show('journal')}}
  function toggleTouchPause(){if($('pause').classList.contains('active'))closeAll(false);else if(!modal)show('pause')}
  function bindMobile(){
    if(!touchMode)return;
    $('control-summary').textContent='Left joystick move · Drag the right side to look · Touch buttons interact, sprint, view evidence, and pause';
    const joystick=$('mobile-joystick'),stick=$('mobile-stick'),look=$('mobile-look-zone'),sprint=$('mobile-sprint');
    let movePointer=null,lookPointer=null,lastLookX=0,lastLookY=0;
    const resetMove=()=>{movePointer=null;mobileMoveX=0;mobileMoveY=0;stick.style.transform='translate(-50%,-50%)'};
    const updateMove=e=>{const r=joystick.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,max=r.width*.34;let dx=e.clientX-cx,dy=e.clientY-cy;const distance=Math.hypot(dx,dy);if(distance>max){dx=dx/distance*max;dy=dy/distance*max}mobileMoveX=dx/max;mobileMoveY=dy/max;stick.style.transform=`translate(calc(-50% + ${dx}px),calc(-50% + ${dy}px))`};
    joystick.addEventListener('pointerdown',e=>{e.preventDefault();movePointer=e.pointerId;joystick.setPointerCapture(e.pointerId);updateMove(e)});
    joystick.addEventListener('pointermove',e=>{if(e.pointerId===movePointer){e.preventDefault();updateMove(e)}});
    ['pointerup','pointercancel','lostpointercapture'].forEach(type=>joystick.addEventListener(type,e=>{if(movePointer===null||e.pointerId===movePointer)resetMove()}));
    look.addEventListener('pointerdown',e=>{if(modal)return;e.preventDefault();lookPointer=e.pointerId;lastLookX=e.clientX;lastLookY=e.clientY;look.setPointerCapture(e.pointerId)});
    look.addEventListener('pointermove',e=>{if(e.pointerId!==lookPointer||modal)return;e.preventDefault();const dx=e.clientX-lastLookX,dy=e.clientY-lastLookY;lastLookX=e.clientX;lastLookY=e.clientY;applyLook(dx,dy,.0052)});
    ['pointerup','pointercancel','lostpointercapture'].forEach(type=>look.addEventListener(type,e=>{if(e.pointerId===lookPointer)lookPointer=null}));
    $('mobile-interact').addEventListener('pointerdown',e=>{e.preventDefault();performInteract()});
    $('mobile-evidence').addEventListener('pointerdown',e=>{e.preventDefault();toggleEvidence()});
    $('mobile-pause').addEventListener('pointerdown',e=>{e.preventDefault();toggleTouchPause()});
    sprint.addEventListener('pointerdown',e=>{e.preventDefault();mobileSprint=true;sprint.classList.add('pressed');sprint.setPointerCapture(e.pointerId)});
    ['pointerup','pointercancel','lostpointercapture'].forEach(type=>sprint.addEventListener(type,()=>{mobileSprint=false;sprint.classList.remove('pressed')}));
  ...
  function bind(){
    ...
    renderer.domElement.addEventListener('click',()=>{if(touchMode||modal)return;if(locked&&current)current.userData.interaction();else renderer.domElement.requestPointerLock()});
    ...
    document.addEventListener('mousemove',e=>{if(touchMode||modal||$('intro').classList.contains('active'))return;if(!locked&&!suppressPause)return;applyLook(e.movementX,e.movementY)});
